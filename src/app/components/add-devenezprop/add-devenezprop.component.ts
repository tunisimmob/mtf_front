...
@Component({
  selector: 'app-add-devenezprop',
  templateUrl: './add-devenezprop.component.html',
  styleUrls: ['./add-devenezprop.component.css']
})
export class AddDevenezpropComponent implements OnInit {

  id: number;
  devenezprop: Devenezproprietaire = new Devenezproprietaire();
  testimage: string;
  ...
  onSubmit() {
    this.DataService.createDevenezProprietaire(this.devenezprop).subscribe(
      (data) => {
        console.log(data);
        this.router.navigate(["list-devprop"]);
      },
      (error) => {console.log(error); this.erreurajout("Vous pouvez pas créer plus qu'une seul information")}
    );
    this.devenezprop = new Devenezproprietaire();
  }
  ...
  uploadFileimg1(file) {
    ...
    this.devenezprop.image1 = this.photourl
  ...
  uploadFileimg2(file) {
    ...
    console.log(this.photourl)
    this.devenezprop.image2 = this.photourll
